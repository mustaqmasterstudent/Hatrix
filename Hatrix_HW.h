 ...
 * @details This will contain the all the functionalities supported by the hardware. It is also imprtant to add the correct hardware file only in this file.
 */
...
#include "V3_Hatrix_HW.h"  /*******************IMPORTANT****************************/  // This include file will change according to the hardware. This is where we have to include the correct hardware file as per the hardware.
    ...
    #endif
    static TIM_TypeDef *Instance = TIM1; // The timer 1 is initialized to use it for general purpose.
    static HardwareTimer *Timer_1 = new HardwareTimer(Instance); // HardwareTimer object is initialized for Timer 1 with pointer because, this should not be destroyed.
#endif
...
/**********************************-----GPIO---START-----*************************************************************************************/
    ...
    class GPIO
    {
        ...
        protected :                  
            ...
            uint8_t                              iGPIOPinStatus                                                   ; // This will store the status of the GPIO pin.
    };                 
#endif
/**********************************-----GPIO-----END-----*************************************************************************************/
/*********************************************************************************************************************************************/
...
/**********************************------PWM----START----*************************************************************************************/
    ...
    class PWM
    {
        public : 
             ...
             *   
             * @param u32Frequency from "0" to "20000". Anywhere between "0" to "20000" is acceptable.  
             * @param u8Dutycycle from "0" to "100". Anywhere between "0" to "100" is acceptable.  
             */  
            void     fnPWMBeginStart               (uint32_t u32Frequency, uint8_t u8Dutycycle)                                        ; 
             ...
             * @param u32Frequency from "0" to "20000". Anywhere between "0" to "20000" is acceptable.  
             */  
            void     fnPWMFrequencyUpdate          (uint32_t u32Frequency)                                                             ; 
        ...
        protected :                                                              
            ...
            uint32_t                               u32PWMFrequency                                                                     ; // This will store the frequency value of the PWM.
            uint8_t                                u8PWMDutycycle                                                                      ; // This will store the dutycycle value of the PWM.
            ...
            #if defined(V2_Hatrix_HW_H)   
                uint32_t                           u32PWMIsenseADCValue[1024]                                                          ; // This will store the raw result from the I_sense pin of the PWM. (upto [1024] values.)
                ...
                uint8_t                            u8PWMNumberOfCyclesToSample         =       156                                     ; // This will say the number of cycles to sample. 
            #endif  
         ...
         * @param u32PWMIsenseADCValue Address of the u32PWMIsenseADCValue[u16PWMCounter] is given and the variable is accesed via pointer.
         ...
         */
        static inline uint32_t fnPWMVersion2Isense(uint8_t * u8IsensePin, uint32_t * u32PWMIsenseADCValue, uint16_t * u16PWMCounter)
        {
            *u32PWMIsenseADCValue = analogRead(*u8IsensePin); // Reads the raw result of ADC of the I_sense pin of the version2 borad. And store it in u32PWMIsenseADCValue[u16PWMCounter].
            *u16PWMCounter++; // We add 1 to the u16PWMCounter for next iteration.
            return *u32PWMIsenseADCValue; // Returns the value of u32PWMIsenseADCValue[u16PWMCounter].
        }
    #endif
#endif
/**********************************------PWM----END------*************************************************************************************/
